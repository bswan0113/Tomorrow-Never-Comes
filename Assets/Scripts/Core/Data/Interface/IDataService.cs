...
namespace Core.Data.Interface
{
    public interface IDataService
    {
        ...
        UniTask CheckSaveDataAsync(int saveSlotId = 1);
        ...
        /// <returns>비동기 저장 작업을 나타내는 UniTask.</returns>
        // CHANGED: saveSlotId 대신 저장할 데이터 객체를 직접 받도록 시그니처 변경
        UniTask SaveAllGameData(GameProgressData gameProgress, PlayerStatsData playerStats);

        // 제네릭 메서드를 통해 특정 타입의 데이터를 로드/저장/삭제
        UniTask<T> LoadDataAsync<T>(int saveSlotId) where T : class;

        /// <summary>
        /// 단일 데이터 엔티티를 저장합니다.
        /// 이 메서드는 내부적으로 자체 트랜잭션을 생성하여 원자성을 보장합니다.
        /// </summary>
        UniTask SaveDataAsync<T>(T data) where T : class;

        UniTask DeleteDataAsync<T>(int saveSlotId) where T : class;
    }
}